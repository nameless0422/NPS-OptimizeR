// Step2Health.jsx - 최종 통합 리팩토링 버전 (예측 + 전략 + 그래프 + 요약)
...
export default function Step2Health({ userInput, onNext }) {
  ...
  const nationalType = expectedDeathAge < 75 ? '조기수령' : expectedDeathAge > 80 ? '연기수령' : '정상수령';
  ...
  const handlePredict = async (formData) => {
    ...
    try {
      ...
      const diseasePenalty = [
        ...
        formData.cancer ? 2 : 0,
        formData.cvd ? 1.5 : 0,
        formData.depression ? 0.5 : 0,
        formData.dementia ? 1.5 : 0,
        formData.hepb ? 0.5 : 0
      ].reduce((a, b) => a + b, 0);
...
const renderSummary = () => {
  ...
  const diseaseCount = ["dm", "cancer", "cvd", "depression", "dementia", "hepb"]
